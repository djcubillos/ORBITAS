       PROGRAM ORBIT
C**********************************************************************
C THIS PROGRAM CALCULATES THE ORBIT OF A SATELLITE WITH C.I.
C THE REV IS CALCULATED WITH THE KEPLER PERIOD   
C INPUT: (table-orb.ini): a(km),e,i(deg),o(Deg),w(Deg),v(Deg),Nºrev
C OUTPUT: (orbit.dib): N,a,e,i,o,u,w,v,x,y,z,dx,dy,dz,time     
C UNITS -- unit adm, rad, 
C         1 unit adm = 6400 Km (R-earth)
c         1 TU = 806.8118744 sec
C**********************************************************************
       IMPLICIT REAL*8 (A-H,O-Z)
       CHARACTER*64 ARXI,ARXO
       ...
       ARXO='orbitaJ254.dib'
       ARXI='table-orb_2.ini'

       WRITE (*,*) 'OUTPUT FILE: ',ARXO
       ...
       NPA=0
10     NPA=NPA+1      
       READ (NCI,*,ERR=200,END=210) (E(J),J=1,6),REV

CCCCCCCCCCC NUMERO DE DATOS	
	XND = 90*REV
CCCCCCCCCCCCCC
...
cccccccccccccccccccccccc
       ...
       T=0.D0
C---------------------------------------------------
C I FIXED A MINIMUN VALUE OF ORBITAL ALTITUDE
C R-earth = 6400 km = 1 UNIT ADIM.
C MINIMUN ORBITAL ALTITUDE = 150 KM
C a = 6550 km = 1.0234375 UNIT ADIM.
C---------------------------------------------------
       RP=E(1)*(1-E(2))
...
100    FORMAT(17E20.12)
        ...
        WRITE (NCO,100) TN,(E(J),J=1,6),(X(J),J=1,6),T!,alx,aly,alz
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC



20     CALL RK78(T,X,N,H,HMIN,HMAX,TOLRK,R,B,F,STIN)           
...
CCCCCCCCCCCCCCCCCCCCCCCCcc
	...
	if ( (TN.ge.(XND/2)).and.(TN.le.((XND/2) + 100)) )THEN 
	WRITE (NCO,100) TN,(E(J),J=1,6),(X(J),J=1,6),T!,alx,aly,alz
	END IF

	IF (TN.eq.XND/2+101) THEN
	WRITE (NCO,100) 
	WRITE (NCO,100) 
	END IF

	if ((TN.ge.(XND - 100)).and.(TN.le.XND))THEN 
	WRITE (NCO,100) TN,(E(J),J=1,6),(X(J),J=1,6),T!,alx,aly,alz
	END IF
	...
	!stop
       ...
       ELSE
C30     H=PER-T
C       CALL RK78(T,X,N,H,HMIN,HMAX,TOLRK,R,B,F,STJ2IN)
C       WRITE (NCO,100) TN,(X(J),J=1,6),T
C100    FORMAT(8E20.12)
C       IF(DABS(H).GT.10.D-15) GOTO 30
       ENDIF
    
40     GOTO 10   

200    WRITE (*,*) 'ERR A LECTURA ARXIU INPUT'
210    CLOSE(NCI)
...
C**********************************************************************
C Camp vectorial equacions inercials satel.lit amb J2
...
C Posar els parametres iguals a totes les rutines que els tinguin !!
        PARAMETER (XMU=1.D0,RE=1.D0,XJ2=0.00108264D0*1)
        DIMENSION X(*),F(*)
        F(1)=X(4)
        F(2)=X(5)
        F(3)=X(6)
	...
	bf = 6.9987D3
	cf = 0.0282146D0
	R0=1.3D0  !densidad (1.3)atmosferica a nivel del mar en kg/m_3
	A=700D3   !altura en m
	!H=8.5E3   !Escala de altura atmosferica en m
	H=8.5D3   !Escala de altura atmosferica en m
	XM=DSQRT((X(1)*X(1))+(X(2)*X(2))+(X(3)*X(3)))*6.4D6 - 6.4D6
	DENSI1 = R0*(exp(-A/H))  
	DENSI2 = R0*(exp(-XM/H))*0+1d-11
	DENSI3 = af*exp(-XM/(bf+cf*XM)) !función parametrizacion
	!write (*,*) af, bf, cf, XM, DENSI3
	!DENSI= 2.23D-16           !en kilogramos sobre metro cubico! 
	CD=2.2D0                   !adimensional!  
	AREA=1.D0                  !en metro cuadrado! 
	XMASA=75.D0                !en kilogramos!
	DCAM=0.5D0*((DENSI3*CD*AREA)/XMASA)*6.4D6
	VREL=DSQRT(X(4)**2 + X(5)**2 + X(6)**2)
        P1=DSQRT((X(1)*X(1))+(X(2)*X(2))+(X(3)*X(3)))
        P2=1.D0/(P1**3.D0)
        CTA=(XMU*RE*RE*XJ2)/(P1**7.D0)
	v= X(1)*X(1) + X(2)*X(2)
	g1=(DCAM)*VREL**2*(X(4)/VREL)
	g2=(DCAM)*VREL**2*(X(5)/VREL)
	g3=(DCAM)*VREL**2*(X(6)/VREL)
        F(4)=-X(1)*XMU*P2+CTA*X(1)*(6.D0*X(3)**2-1.5D0*v)-g1
        F(5)=-X(2)*XMU*P2+CTA*X(2)*(6.D0*X(3)**2-1.5D0*v)-g2
        F(6)=-X(3)*XMU*P2+CTA*X(3)*(3.D0*X(3)**2-4.5D0*v)-g3
	!WRITE (200,*) P1,VREL,g1**2+g2**2+g3**2 
 	!WRITE (*,*) F(4), F(5), F(6)
	!WRITE (*,*) "DENSIDAD1 = ", DENSI1
	!WRITE (*,*) "DENSIDAD2 = ", DENSI2, XM
	!write (*,*) DENSI3
	!STOP
        RETURN
        END
...
C********************************************************************
     ...
     1),BETA(79),C(11),CP(13),D,DD,E3,E4,HMI,HMAX,FAC
         DATA II/0/
